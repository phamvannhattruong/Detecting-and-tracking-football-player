...
class Tracker:
        ...
        for frame_nums, detection in enumerate(detections):
            ...
            for frame_detection in detection_with_track:
                ...
                if cls_id == cls_name_inv['player']:
                    tracks["player"][frame_nums][track_id] = {"bbox":bbox}

                if cls_id == cls_name_inv['referee']:
                    tracks["referee"][frame_nums][track_id] = {"bbox":bbox}

                if cls_id == cls_name_inv['ball']:
                    tracks["ball"][frame_nums][track_id] = {"bbox":bbox}
            ...
            for frame_detection in detection_supervision:
                ...
                if cls_id == cls_name_inv['ball']:
                    tracks["ball"][frame_nums][1] = {"bbox": bbox}
    ...
    def draw_annotation(self, video_frame, tracks):
        output_video_frames = []

        for frame_nums, frame in enumerate(video_frame):
            frame = frame.copy()

            player_dict = tracks["player"][frame_nums]
            ball_dict = tracks["ball"][frame_nums]
            referee_dict = tracks["referee"][frame_nums]
